...
- **Авторизация**: если провайдер требует вход в аккаунт (редирект на login, blocked state и т.д.), агент НЕ должен пытаться обойти это. Вместо этого агент останавливается и сообщает пользователю: "Провайдер X требует авторизацию. Пожалуйста, войдите в аккаунт в браузере."
- **GLM версия по умолчанию**: основная версия GLM — `glm-5.2`, не `glm-4-plus` или другая. При выборе модели в запросах к GLM использовать `glm-5.2`.
- **Разделение вкладок**: каждый провайдер (GLM, Qwen, DeepSeek, Kimi) открывается и работает в отдельной вкладке браузера. НЕ переиспользовать одну вкладку для нескольких провайдеров. Использовать `browser_navigate` для создания/переключения между вкладками.
